...
def delete_transactions(user: user_dependency, db: db_dependency, transaction_id: int):
    ...
    db.query(Transaction).filter(Transaction.owner_id == user.get('id')).filter(Transaction.id == transaction_id).delete()
    db.commit()
...
def get_user(user: user_dependency, db: db_dependency):
    ...
    if user is None:
        raise HTTPException(status_code=401, detail='Failed Authenticaion')
